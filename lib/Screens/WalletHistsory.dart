...
class StateWallet extends State<WalletHistory> with TickerProviderStateMixin {
  ...
  TextEditingController? acc_num;
  TextEditingController? ifsc_code;
  TextEditingController? acc_name;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              if (tranList[index].opnBal != "" &&
                      tranList[index].opnBal != null &&
                      tranList[index].opnBal!.isNotEmpty) Text(
                      "${getTranslated(context, OPNBL_LBL)!} : ${tranList[index].opnBal!}",
                    ) else Container(),
              if (tranList[index].clsBal != "" &&
                      tranList[index].clsBal != null &&
                      tranList[index].clsBal!.isNotEmpty) Text(
                      "${getTranslated(context, CLBL_LBL)!} : ${tranList[index].clsBal!}",
                    ) else Container(),
              if (tranList[index].msg != "" && tranList[index].msg!.isNotEmpty) Text(
          ...
          if (!error) {
            ...
            if (offset < total) {
              ...
              tempList = (data as List)
                  .map((final data) => TransactionModel.fromJson(data))
                  .toList();
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            ...
            if (offset < total) {
              ...
              final data = getdata["data"];
              tempList = (data as List)
                  .map((final data) => TransactionModel.fromReqJson(data))
                  .toList();
